...
tidy_PHM <- function(x,primary_segment,secondary_segment){
  ...
  x<-x[,-ncol(x)]
  colnames(x)[1]<-"METRIC"
  x <- x[x$METRIC!="PCN" & x$METRIC!="Practice" & !is.na(x$METRIC) & x$METRIC!="Ethnicity Sub-Group" & !is.na(suppressWarnings(as.numeric(x$`Whole population for NHS Lincolnshire ICB`))),]
